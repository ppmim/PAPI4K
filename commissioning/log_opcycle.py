...
# Log instrument operation cycles, create FITS header entries, copy log
# file to other panic computer
# 
# This script provides an easy way to add operation cycles in the hardware
# log file. It also has functions to create the corresponding FITS header
# entries for GEIRS, and copy the log file to the other panic computer. For
# more details about the logs, consult the documentation in PANIC-SW-SP-01.
#
# usage: log_opcycle.py [-h] [-f | -c] [--logpath LOGPATH] [--logfile LOGFILE]
#                       [--fitspath FITSPATH] [--fitsfile FITSFILE]
#                       [--targetuser TARGETUSER] [--targethost TARGETHOST]
# 
# Script to edit the PANIC operation cycle log, create the corresonding FITS
# header keyword inputs for GEIRS, and copy the log file to the other panic
# computer
# 
# optional arguments:
...
# 1.2 01/04/2015 BD Update
#	1) Added check of number of entries when loading logfile
#	2) Changed print of log entries to 5 or maximum available lines
#	3) New cycle with maximum number + 1
#	4) Clarified manual time edit
# 1.2.1 01/04/2015 BD Update
...
parser = argparse.ArgumentParser(description='Script to edit the PANIC operation cycle log, create the corresonding FITS header keyword inputs for GEIRS, and copy the log file to the other panic computer')
...
def loadlogfiledata(logfilepath):
	...
	if not os.path.exists(logfilepath):
		raise IOError('Operation log file {} not found!'.format(logfilepath))
	else:
		nrs = []
		dates = []
		times = []
		...
		for line in lines:
			...
			try:
				...
				raise ValueError('Invalid number entry "{}" in operation log, type must be integer'.format(nr))
			nrs.append(int(nr))
			date = tokens[1].strip()
			try:
				datetime.datetime.strptime(date, '%Y-%m-%d')
			except ValueError:
				raise ValueError('Invalid date entry "{}" in operation log, format must be YYYY-mm-dd'.format(date))
			time = tokens[2].strip()
			try:
				datetime.datetime.strptime(time, '%H:%M')
			except ValueError:
				raise ValueError('Invalid time entry "{}" in operation log, format must be HH:MM'.format(time))
			dates.append(date)
			times.append(time)
			comments.append(tokens[3].strip())
		logfile.close()
		if len(nrs) == 0:
			raise IOError('No valid entries found in operation log file {}'.format(logfilepath))
...
if verbose:
	...
	print('# Loading operation logfile data')
...
if not fitsonly and not copyonly:
	...
	print('(A) Add new cycle, create FITS header data, copy log file')
	print('(F) Only create FITS header data from last entry')
...
if action in ['F', 'A']:
	...
	# Get last entry if not added before, verify format
	if action == 'F':
		nr = nrs[-1]
		date = dates[-1]
		time = times[-1]
	...
	# lines for files
	cyclkey = "OPCYCL = {} / {}\n".format(nr, 'Operation cycle number')
	datekey = "OPDATE = '{}' / {}\n".format(isodate, 'UT-date of operation cycle start')
	if os.path.exists(FITSfilepath):
		...
		for line in lines:
			...
			if line.strip().startswith('OPCYCL'):
				line = cyclkey
				cyclwritten = True
			if line.strip().startswith('OPDATE'):
				line = datekey
				datewritten = True
...
if action in ['A', 'C']:
	# copy log file to other panic computer
	...
	if not targetuser:
		if hostname == 'panic22':
			...
			raise ValueError('Unknow local host name (not "panic22" or "panic35"), cannot determine target user name, please specify in input')
