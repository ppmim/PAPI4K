#!/usr/bin/env python
#########################################################################
# PANIC commissioning V2 (Dic-2022/Jan-2023)
...
# files.
#
# usage: p_52_tiltcheck.py [-h] [-i INPUT] [-o OUTPUT] [--nolog] {1,2,3,4,5,6,7}
# 
...
# 1.3 16/11/2022 JM Update for PANICv2
#   1) TBD modifications to use data from StarSimulator

_name = 'p_52_tiltcheck.py'
_version = '1.3'
#########################################################################
import numpy as np
import os
...
M2scale = 0.23 # StarSimulator scale 
...
def loadfiledata_SS(filter):
    """Load data of input file of a single filter for StarSimulator
    Input
    -----
    datafilename : str
             Name of file with data (x,y,Focus)
    Returns
    -------
    obj : str
          Name of the object
    ...
    focus = np.ma.masked_all(16)
    obj = '--'
    data = np.loadtxt(inputpath, ndmin=2, encoding='UTF-8')
    obj = "StarSim"
    for r in range(0, len(data)):
        idx = regionindex(data[r,:][0], data[r,:][1])
        pxi[idx] = data[r, :][0] 
        pxj[idx] = data[r, :][1] 
        focus[idx] = data[r,:][2]
        subquads[idx] = 'R_%1i' %(idx)
    
    return obj, subquads, pxi, pxj, focus



def convertdata_2014(pxi, pxj, focus):
...
def convertdata(pxi, pxj, focus):
    ...
    # FPAx = (-100.5 + pxi) * 0.015 # pixel size 0.015um for H4RG
    FPAy = pxj.copy()
    FPAy = (-2048.5 + pxj) * 0.015
    # FPAy = (-100.5 + pxj) * 0.015
    FSx = FPAx / FPAscale
    FSy = FPAy / FPAscale
    FSz = focus * M2scale
    return FPAx, FPAy, FSx, FSy, FSz
...
if ifilter in range(1, 7):
    ...
    obj, subquads, pxi, pxj, focus = loadfiledata_SS(filter)
    if pxi.count() == 0:
    ...
    FPAx, FPAy, FSx, FSy, FSz = convertdata_2014(pxi, pxj, focus)
    pr('# Parameters:')
